...
<table width="100%" border="0" cellspacing="0" cellpadding="0" style="margin-top:10px;">
	...
	<tr>
		...
		<td class="ct_line02"></td> <!-- 상태 앞 구분선 추가 -->
  		<td class="ct_list_b">상태</td>				
	</tr>
	<tr>
		<td colspan="9" bgcolor="808285" height="1"></td> <!-- colspan 9로 맞춤 -->
	</tr>
...
%>
	<tr class="ct_list_pop">
		...
		<td align="left">
			...
			<% if (link.isEmpty()) { %> 
				<%= vo.getProdNo() %> <!-- 링크 제거된 경우 그냥 텍스트 출력 -->
			<% } else { %>
				<a href="<%=link%>"><%= vo.getProdNo() %></a> <!-- 링크 있는 경우 -->
			<% } %>
		</td>
		<td></td>
		...
		<td></td>  <!-- 상태 앞 빈 칸 구분선 -->
  		<td align="left"><%= vo.getProTranCode() %></td> <!-- 상태 표시 -->
  		<!-- <td align="left"><%= vo.getProTranCode() %></td>  --><!-- 상태 표시 -->		
	</tr>
	<tr>
	...
	<tr>
		...
		%>
			<!-- <a href="/listProduct.do?page=<%=i%>"><%=i %></a>  -->
			 <a href="/listProduct.do?page=<%=i%>&searchCondition=<%=searchVO.getSearchCondition()%>&searchKeyword=<%=searchVO.getSearchKeyword()%>"><%=i %></a>
		<%
			}
		%>	
    	</td>

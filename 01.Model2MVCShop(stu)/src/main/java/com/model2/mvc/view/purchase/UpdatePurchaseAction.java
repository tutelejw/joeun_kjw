...
public class UpdatePurchaseAction extends Action {
    ...
    public String execute(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        PurchaseVO sessionPurchaseVO = (PurchaseVO) session.getAttribute("vo");

        // 세션에 PurchaseVO 객체가 없으면 새로 저장
        if (sessionPurchaseVO == null) {
            System.out.println("No PurchaseVO found in session, adding new object.");
            session.setAttribute("vo", vo);  // 세션에 PurchaseVO 객체 저장
        } else {
            System.out.println("Found existing PurchaseVO in session.");
        }

        // 세션에서 tranNo에 해당하는 구매정보를 갱신
        int sessionId = sessionPurchaseVO != null ? sessionPurchaseVO.getTranNo() : -1;
        System.out.println("Session transaction number: " + sessionId);  // 디버깅용 출력

        // tranNo가 일치하면 세션의 구매정보를 갱신
        if (sessionId == tranNo) {
            session.setAttribute("vo", vo); // 세션에 업데이트된 PurchaseVO 저장

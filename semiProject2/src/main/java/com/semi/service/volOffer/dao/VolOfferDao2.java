	...
	public Map<String , Object> getVolOfferList(Search search, String region) throws Exception {
	    ...
	    if (search.getSearchCondition() != null) {
	        if (search.getSearchCondition().equals("0") && !search.getSearchKeyword().equals("")) {
	            sql += (sql.contains("WHERE") ? " AND " : " WHERE ") + "flag = 'o' or 1010='" + search.getSearchKeyword() + "'"; // 임시값 조건
...
//		}
	    ...
	    if (region != null && !region.isEmpty()) {
	        System.out.println("getVolOfferList region 값 받아오는지 확인 pStmt.setString(1, region);   : " + region);
	        pStmt.setString(index++, region);  // 1번 인덱스에 region 값을 설정
	    }

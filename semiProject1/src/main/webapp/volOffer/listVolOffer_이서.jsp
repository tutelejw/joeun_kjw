...
<c:set var="items"        value="${not empty list ? list : volOfferList}" />
<!-- <c:set var="items"        value="${not empty volOfferList ? volOfferList : list}" /> -->
...
<div class="topbar">
  <div class="container">
    ...
    <div class="nav">
      ...
      <!-- 손길나눔 현재 페이지 -->
      <a href="/listVolOffer.do" class="active"><span>손길나눔</span></a>
      <a href="#"><span>햇살소식</span></a>
      <div class="dropdown">
...
<main class="container-main">
  <section class="panel">
    ...
    <div class="panel-header">
      <div class="panel-title">손길나눔(봉사제공)</div>
      <form class="searchbar" method="get" action="<c:url value='/listVolOffer.do'/>">
      ...
      <c:choose>
        <c:when test="${not empty items}">
          <table>
            <thead>
              <tr>
                <th style="width:120px;text-align:center">번호</th>
                <th>제목</th>
            ...
            <tbody>
               ...
               <c:forEach var="item" items="${items}">
                <tr>
                  <td style="text-align:center">${item.postId}</td>
                  <td class="title"><a href="<c:url value='/getVolOffer.do'><c:param name='postId' value='${item.postId}'/></c:url>">${item.title}</a></td>
      ...
      <div class="pagination">
        <c:if test="${totalPage > 1}">
          ...
          <c:if test="${startPage > 1}">
            <a href="<c:url value='/listVolOffer.do'><c:param name='currentPage' value='${startPage-1}'/><c:param name='pageSize' value='${pageSize}'/><c:param name='q' value='${q}'/></c:url>">이전</a>
          </c:if>
          <c:forEach var="p" begin="${startPage}" end="${endPage}">
            <c:choose>
              ...
              <c:otherwise>
                <a href="<c:url value='/listVolOffer.do'><c:param name='currentPage' value='${p}'/><c:param name='pageSize' value='${pageSize}'/><c:param name='q' value='${q}'/></c:url>">${p}</a>
              </c:otherwise>
            </c:choose>
          </c:forEach>
          <c:if test="${endPage < totalPage}">
            <a href="<c:url value='/listVolOffer.do'><c:param name='currentPage' value='${endPage+1}'/><c:param name='pageSize' value='${pageSize}'/><c:param name='q' value='${q}'/></c:url>">다음</a>
          </c:if>
        </c:if>
      </div>

      <a class="btn primary" href="<c:url value='/addVolOffer.do'/>">등록</a>
    </div>
  </section>
</main>
